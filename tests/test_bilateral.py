"""Testes da estabilização esquerda/direita das pernas."""
...
def test_bilateral_tracker_corrects_left_right_chain_swap():
    ...
    for left, right in (
        ...
        swapped.keypoints[left_index], swapped.keypoints[right_index] = (
            swapped.keypoints[right_index],
            swapped.keypoints[left_index],
        )
    ...
    assert abs(
        corrected[int(BodyKeypoint.LEFT_KNEE)].x
        - reference[int(BodyKeypoint.LEFT_KNEE)].x
    ) < 1e-6
    assert abs(
        corrected[int(BodyKeypoint.RIGHT_ANKLE)].x
        - reference[int(BodyKeypoint.RIGHT_ANKLE)].x
    ) < 1e-6


def test_bilateral_tracker_rejects_duplicate_leg_after_clear_separation():
    tracker = BilateralLegIdentityTracker(
    ...
    collapsed.keypoints[right_knee] = Keypoint(
        reference[left_knee].x,
        reference[left_knee].y,
        0.90,
    )
    collapsed.keypoints[right_ankle] = Keypoint(
        reference[left_ankle].x,
        reference[left_ankle].y,
        0.90,
    )
...
def test_bilateral_tracker_does_not_reject_close_legs_without_previous_separation():
    ...
    left_knee = int(BodyKeypoint.LEFT_KNEE)
    right_knee = int(BodyKeypoint.RIGHT_KNEE)
    close.keypoints[right_knee] = Keypoint(
        close[left_knee].x + 0.005,
        close[left_knee].y,
        1.0,
    )
