...
from src.pose.decoder import (
    decode_heatmaps,
    decode_heatmaps_bilateral,
    decode_heatmaps_reliable,
)
...
def test_generate_heatmaps():
    ...
    visibility[0] = 1.0
    heatmaps = generate_heatmaps(keypoints, visibility, heatmap_size=64, sigma=1.8)
...
def test_reliable_decoder_reduces_ambiguous_peak_confidence():
    heatmaps = torch.zeros((1, NUM_KEYPOINTS, 64, 64), dtype=torch.float32)
    knee = int(BodyKeypoint.LEFT_KNEE)
    heatmaps[0, knee, 40, 20] = 0.80
    heatmaps[0, knee, 40, 44] = 0.76

    pose, report = decode_heatmaps_reliable(
        heatmaps,
        suppression_radius=3,
        full_confidence_dominance=0.35,
    )

    assert pose[knee].confidence < 0.80
    assert pose[knee].confidence > 0.0
    assert report.ambiguous_keypoints >= 1


def test_reliable_decoder_keeps_isolated_peak_confidence():
    heatmaps = torch.zeros((1, NUM_KEYPOINTS, 64, 64), dtype=torch.float32)
    ankle = int(BodyKeypoint.RIGHT_ANKLE)
    heatmaps[0, ankle, 52, 42] = 0.80

    pose, _ = decode_heatmaps_reliable(heatmaps, suppression_radius=3)
    assert abs(pose[ankle].confidence - 0.80) < 1e-5
...
def test_bilateral_decoder_does_not_promote_weak_noise():
    ...
    pose, report = decode_heatmaps_bilateral(heatmaps, minimum_alternative_ratio=0.65)
