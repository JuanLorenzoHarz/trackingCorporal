...
def test_positive_heatmap_weight_emphasizes_keypoint_peak():
    ...
    old_style = masked_heatmap_loss(prediction, target, visibility, positive_weight=0.0)
    weighted = masked_heatmap_loss(prediction, target, visibility, positive_weight=8.0)
...
def test_invisible_keypoint_does_not_affect_loss():
    ...
    loss = masked_heatmap_loss(prediction, target, visibility, positive_weight=8.0)
    assert float(loss.item()) == 0.0


def test_negative_sample_supervises_all_heatmaps_to_zero():
    prediction = torch.full((1, NUM_KEYPOINTS, 8, 8), 0.25, dtype=torch.float32)
    target = torch.zeros_like(prediction)
    supervision = torch.ones((1, NUM_KEYPOINTS, 1, 1), dtype=torch.float32)
...
def test_bilateral_loss_penalizes_both_knees_on_same_target():
    ...
    correct_loss = bilateral_cross_peak_loss(correct, target, visibility, minimum_target_distance=3.0)
    collapsed_loss = bilateral_cross_peak_loss(collapsed, target, visibility, minimum_target_distance=3.0)
...
def test_bilateral_loss_ignores_overlapping_ground_truth():
    ...
    loss = bilateral_cross_peak_loss(prediction, target, visibility, minimum_target_distance=3.0)
    assert float(loss.item()) == 0.0


def test_bilateral_loss_ignores_negative_zero_targets():
    target = torch.zeros((1, NUM_KEYPOINTS, 16, 16), dtype=torch.float32)
    supervision = torch.ones((1, NUM_KEYPOINTS, 1, 1), dtype=torch.float32)
    prediction = torch.full_like(target, 0.2)
